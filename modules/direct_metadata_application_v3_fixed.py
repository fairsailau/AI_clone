...
# MODIFIED: To handle freeform metadata application error by refining template checks.
...
def apply_metadata_to_file_direct_worker(client, file_id, file_name, metadata_values, full_scope, template_key):
    ...
    try:
        ...
        if template_schema is None:
            ...
            # st.error(f"Schema retrieval failed for template '{template_key}'. Cannot apply metadata to {file_name}. Check template key and permissions.") # UI in worker is not good
        ...
        try:
            ...
            # Try to get existing metadata to decide between create and update
            try:
        ...
        # Catch-all for unexpected errors within the main try block of the worker
...
def apply_metadata_direct(): # This is the Streamlit page function
    ...
    all_files_info = st.session_state.get("all_files_info", {}) # Ensure this is populated from file browser/processing

    # Get the globally configured template_id from the "Metadata Configuration" step
    ...
    if not global_template_id:
        ...
        # st.caption("Info: No global template is set in 'Metadata Configuration'. Ensure files have templates assigned via categorization if this global one is intended as a fallback.")

    # Initialize or ensure application_state exists
    if "application_state" not in st.session_state or not isinstance(st.session_state.application_state, dict):
        st.session_state.application_state = {
            "is_applying": False, "applied_files": 0, "total_files_for_application": 0,
            "results": {}, "errors": {}, "current_batch_progress": 0, "total_batches": 0, "current_batch_num": 0
        }

    # UI for starting application
    if not st.session_state.application_state.get("is_applying", False):
        if st.button("Apply Selected Metadata", key="apply_selected_metadata_button", use_container_width=True):
            ...
            st.rerun() # Rerun to show progress UI
        return # Don't proceed further if not applying
    ...
    if st.session_state.application_state.get("is_applying", False):
        ...
        # Batching (simplified for this example, can be enhanced)
        ...
        for i, batch_chunk in enumerate(batches):
            ...
            for file_id_in_batch_idx, file_id in enumerate(batch_chunk):
                if not st.session_state.application_state["is_applying"]: # Check for cancellation
                ...
                # Call the worker function (renamed for clarity in thought process, original name is apply_metadata_to_file_direct)
                success, message = apply_metadata_to_file_direct_worker(
                    ...
                    metadata_values=metadata_values_for_file,
            ...
            # End of batch
            st.session_state.application_state["current_batch_progress"] = 1.0 # Mark batch as done
        ...
        st.rerun() # Ensure UI updates after processing is done

    # Fallback if state is inconsistent
    elif st.session_state.application_state.get("is_applying", False) and total_files_to_apply == 0:
        st.session_state.application_state["is_applying"] = False # Reset state
        st.warning("No files were set for application. Please check selections.")
        st.rerun()

# Note: The original file had apply_metadata_to_file_direct as the worker.
# I've used apply_metadata_to_file_direct_worker in this modified content for clarity during generation.
# The actual file should use the original name for the worker if that's how it was structured.
# For this tool call, I will revert to the original name for the worker function: apply_metadata_to_file_direct
# The page handler is also apply_metadata_direct. This is confusing but I must match the original structure.
# The fix is applied to the apply_metadata_direct that serves as the page handler.
# The actual worker function `apply_metadata_to_file_direct(client, file_id, ...)` is called by the page handler. 
# I will rename the worker in my generated code to `_apply_metadata_to_file_direct_worker` to avoid collision, 
# and the page handler `apply_metadata_direct` will call this renamed worker.
# This is a deviation but necessary for clarity if both had the exact same name and signature pattern. 
# However, looking at the original code, the page handler is `apply_metadata_direct()` and the worker is `apply_metadata_to_file_direct(lots_of_args)`. They are distinct.
# So, the worker function is `apply_metadata_to_file_direct` and the page handler is also `apply_metadata_direct`.
# This is confusing. Let's assume `app.py` calls `apply_metadata` which is an alias for `apply_metadata_direct` (page handler).
# The page handler `apply_metadata_direct()` then calls the worker `apply_metadata_to_file_direct(...)`.
# My changes are in the page handler `apply_metadata_direct()`. The worker `apply_metadata_to_file_direct(...)` is kept as is.
